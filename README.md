...
Pedagogical Goal: Cartesian Coordinate Geometry (navigating an X, Y plane) and basic spatial awareness.

Applied Theory: Experiential Learning (learning by doing) and Constructivism (testing inputs, seeing immediate consequences).

Core Mechanic (Facilitating Learning): The player targets enemies using specific grid coordinates through UI Input Fields. A correct intersection of X and Y axes results in a successful hit (positive reinforcement); an incorrect input wastes an Action Point (AP) and misses, teaching careful calculation.